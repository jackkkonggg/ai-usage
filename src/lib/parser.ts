...
export function calcCost(model: string, input: number, output: number, cacheRead: number, cacheWrite: number) {
...
export function parseClaudeFile(filePath: string): { turns: Turn[]; glmSessionIds: string[] } {
  ...
  let content = ''
  try { content = readFileSync(filePath, 'utf-8') } catch { return { turns, glmSessionIds } }
  ...
  for (const line of content.split('\n')) {
    ...
    try {
      ...
      if (model.startsWith('glm')) { glmSessionIds.push(sessionId); continue }
...
export function parseCodexFile(filePath: string): { turns: Turn[]; project: string | null } {
  ...
  let content = ''
  try { content = readFileSync(filePath, 'utf-8') } catch { return { turns, project } }

  let prevInput = 0, prevOutput = 0, prevCached = 0
  ...
  for (const line of content.split('\n')) {
    ...
    try {
      ...
      if (deltaInput > 0 || deltaOutput > 0) {
        const ts: number = obj.timestamp ? new Date(obj.timestamp).getTime()
          : typeof obj.ts === 'number' ? obj.ts : Date.now()
        turns.push({
          source: 'codex', sessionId,
          date: new Date(ts).toISOString().slice(0, 10), timestamp: ts,
          model: currentModel, inputTokens: deltaInput, outputTokens: deltaOutput,
          cacheReadTokens: deltaCached, cacheWriteTokens: 0,
          costUsd: calcCost(currentModel, deltaInput, deltaOutput, deltaCached, 0),
        })
      }
      prevInput = currInput; prevOutput = currOutput; prevCached = currCached
    } catch { /* skip */ }
  }
...
export interface HistoryEntry {
  display: string; pastedContents: Record<string, unknown>
  timestamp: number; project: string; sessionId: string
}
