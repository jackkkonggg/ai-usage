...
interface Summary {
  totalCost: number;  todayCost: number;  weekCost: number;  monthCost: number
  totalTokens: number; claudeTurns: number; codexTurns: number; totalSessions: number
  claudeFound: boolean; codexFound: boolean
  totalMessages: number | null; timeSavedMs: number
  firstSessionDate: string | null; cacheHitRate: number | null
}
...
interface Session {
  sessionId: string; source: 'claude' | 'codex'; date: string; lastTimestamp?: number
  model: string; cost: number; tokens: number; turns: number
}
interface ModelStat {
  model: string; source: string; cost: number; tokens: number; turns: number
}
interface HourlyEntry  { hour: number; label: string; count: number }
interface ProjectStat  { project: string; displayName: string; sessionCount: number; messageCount: number; lastActive: string }
interface ActivityEntry { date: string; messages: number; sessions: number; toolCalls: number }

// ─── Helpers ──────────────────────────────────────────────────────────────────

const fmt$ = (n: number) =>
  n === 0 ? '$0.00' : n < 0.005 ? '<$0.01' : `$${n.toLocaleString('en-US', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
const fmtBig$ = (n: number) =>
  n >= 1000 ? `$${(n / 1000).toLocaleString('en-US', { minimumFractionDigits: 1, maximumFractionDigits: 1 })}k` : fmt$(n)
...
function ChartTooltip({ active, payload, label }: {
  ...
  return (
    <div style={{
      ...
      fontFamily: C.mono, fontSize: 12, lineHeight: 1.8,
      boxShadow: '0 8px 24px rgba(0,0,0,0.5)',
    }}>
      <div style={{ color: C.textDim, marginBottom: 4, fontSize: 11 }}>{label}</div>
      {c > 0 && <div style={{ color: C.orange }}>Claude  <span style={{ float: 'right', marginLeft: 24 }}>{fmt$(c)}</span></div>}
      {x > 0 && <div style={{ color: C.emerald }}>Codex   <span style={{ float: 'right', marginLeft: 24 }}>{fmt$(x)}</span></div>}
      <div style={{ color: C.amber, borderTop: `1px solid ${C.border}`, marginTop: 4, paddingTop: 4 }}>
...
function ActivityTooltip({ active, payload, label }: {
  ...
  return (
    <div style={{
      ...
      <div style={{ color: C.textDim, marginBottom: 4, fontSize: 11 }}>{label}</div>
      <div style={{ color: C.amber }}>Messages   <span style={{ float: 'right', marginLeft: 24 }}>{msgs}</span></div>
      <div style={{ color: C.orange }}>Tool Calls <span style={{ float: 'right', marginLeft: 24 }}>{tools}</span></div>
      <div style={{ color: C.muted }}>Sessions   <span style={{ float: 'right', marginLeft: 24 }}>{sess}</span></div>
    </div>
  )
}
...
function StatCard({ label, value, sub }: { label: string; value: string; sub?: string }) {
  return (
    <div style={{
      ...
      <span style={{ fontSize: 11, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.09em', fontWeight: 500 }}>
        {label}
      </span>
      <span style={{ fontSize: 30, fontWeight: 700, fontFamily: C.mono, color: C.amber, letterSpacing: '-1.5px', lineHeight: 1 }}>
        {value}
      </span>
...
export default function Dashboard() {
  ...
  const fetchAll = useCallback(async (days = range) => {
    setRefreshing(true)
    try {
      ...
      setSummary(s); setDaily(d); setSessions(sess); setModels(m)
      setHourly(h); setProjects(p); setActivity(a)
    } catch (err) {
      console.error(err)
    } finally {
      setLoading(false)
  ...
  const handleRange = (days: number) => { setRange(days); fetchAll(days) }
    ...
    return (
      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', height: '100vh' }}>
        <div style={{ textAlign: 'center' }}>
          ...
          <div style={{ color: C.muted, fontFamily: C.mono, fontSize: 13 }}>parsing session data…</div>
  ...
  return (
    <div style={{ background: C.bg, minHeight: '100vh', padding: '28px 36px 48px', maxWidth: 1600, margin: '0 auto' }}>

      {/* Header */}
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 36 }}>
        <div>
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            <h1 style={{ fontSize: 22, fontWeight: 700, letterSpacing: '-0.5px', color: C.text }}>AI Subscription Usage</h1>
            <span style={{ width: 8, height: 8, borderRadius: '50%', background: C.amber, boxShadow: `0 0 8px ${C.amber}`, display: 'inline-block' }} />
          </div>
        ...
        <button
          ...
          style={{
            ...
            padding: '8px 18px', borderRadius: 7, cursor: refreshing ? 'default' : 'pointer',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 12, marginBottom: 12 }}>
        <StatCard label="All Time"     value={fmtBig$(s.totalCost)}  sub={`${fmtTokens(s.totalTokens)} tokens`} />
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12, marginBottom: 24 }}>
        {[
          ...
          <div key={item.label} style={{
            ...
            <div>
              <div style={{ fontSize: 11, color: C.muted, textTransform: 'uppercase', letterSpacing: '0.09em', marginBottom: 4 }}>
                {item.label}
              </div>
              <div style={{ fontSize: 24, fontWeight: 700, fontFamily: C.mono, color: C.text, letterSpacing: '-1px' }}>
                {item.value}
              </div>
            </div>
          </div>
      ...
      <div style={{
        background: C.surface, border: `1px solid ${C.border}`,
        borderRadius: 12, padding: '22px 28px', marginBottom: 20,
      }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 24 }}>
          ...
          <div style={{ display: 'flex', gap: 6 }}>
            {[7, 30, 90].map(d => (
              <button key={d} onClick={() => handleRange(d)} style={{
        ...
        {chartTab === 'cost' ? (
          <>
            <ResponsiveContainer width="100%" height={180} style={{ outline: 'none' }}>
              <BarChart data={costChartData} barSize={range <= 7 ? 24 : range <= 30 ? 12 : 5} barGap={1}>
            ...
            <div style={{ display: 'flex', gap: 20, marginTop: 14, justifyContent: 'flex-end' }}>
              {[{ color: C.orange, label: 'Claude Code' }, { color: C.emerald, label: 'OpenAI Codex' }].map(l => (
                <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, color: C.textDim }}>
            ...
            <ResponsiveContainer width="100%" height={180} style={{ outline: 'none' }}>
              <AreaChart data={activityChartData}>
                ...
                <XAxis dataKey="date"
                  tick={{ fill: C.muted, fontSize: 11, fontFamily: 'JetBrains Mono, monospace' }}
                  axisLine={false} tickLine={false}
                  interval={activityChartData.length <= 7 ? 0 : activityChartData.length <= 30 ? 4 : 9}
                />
                <YAxis
                  tick={{ fill: C.muted, fontSize: 11, fontFamily: 'JetBrains Mono, monospace' }}
                  axisLine={false} tickLine={false}
                ...
                <Area type="monotone" dataKey="messages"  stroke={C.amber}  fill={C.amber}  fillOpacity={0.15} strokeWidth={2} />
                <Area type="monotone" dataKey="toolCalls" stroke={C.orange} fill={C.orange} fillOpacity={0.15} strokeWidth={2} />
              </AreaChart>
            </ResponsiveContainer>
            <div style={{ display: 'flex', gap: 20, marginTop: 14, justifyContent: 'flex-end' }}>
              {[{ color: C.amber, label: 'Messages' }, { color: C.orange, label: 'Tool Calls' }].map(l => (
                <div key={l.label} style={{ display: 'flex', alignItems: 'center', gap: 7, fontSize: 12, color: C.textDim }}>
                  <span style={{ width: 10, height: 10, borderRadius: 2, background: l.color, display: 'inline-block' }} />
                  {l.label}
                </div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '360px 1fr', gap: 16 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: C.textDim, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 18 }}>
              Activity by Hour
            </div>
          ...
          <div style={{
            ...
            <div style={{ fontSize: 12, fontWeight: 600, color: C.textDim, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 22 }}>
              By Project
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  ...
                  <div key={p.project}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 14, color: C.text, fontWeight: 500 }}>{p.displayName}</span>
                      </div>
                      <span style={{ fontSize: 13, fontFamily: C.mono, color: C.amber, fontWeight: 600 }}>{fmtTokens(p.messageCount)}</span>
                    </div>
              ...
              {projects.length === 0 && (
                <div style={{ color: C.muted, fontSize: 13, textAlign: 'center', padding: '20px 0' }}>No project data found</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
                  ...
                  <div key={m.model}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <span style={{ fontSize: 14, color: C.text, fontWeight: 500 }}>{shortModel(m.model)}</span>
                      </div>
                      <span style={{ fontSize: 13, fontFamily: C.mono, color: C.amber, fontWeight: 600 }}>{fmt$(m.cost)}</span>
                    </div>
                    <div style={{ height: 3, background: C.border, borderRadius: 2, overflow: 'hidden', marginBottom: 5 }}>
              ...
              {models.length === 0 && (
                <div style={{ color: C.muted, fontSize: 13, textAlign: 'center', padding: '20px 0' }}>No model data found</div>
              )}
            </div>
          </div>
          ...
          <div style={{
            background: C.surface, border: `1px solid ${C.border}`,
            borderRadius: 12, padding: '22px 28px', overflow: 'hidden',
          }}>
            <div style={{ fontSize: 12, fontWeight: 600, color: C.textDim, textTransform: 'uppercase', letterSpacing: '0.1em', marginBottom: 18 }}>
              Recent Sessions
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {sessions.slice(0, 50).map((sess, i) => (
                    ...
                    >
                      <td style={{ padding: '11px 16px 11px 0', color: C.textDim, fontFamily: C.mono, fontSize: 12 }}>{fmtSessionDate(sess.date, sess.lastTimestamp)}</td>
                      <td style={{ padding: '11px 16px 11px 0' }}><SourceBadge source={sess.source} /></td>
                      <td style={{ padding: '11px 16px 11px 0', color: C.text, fontFamily: C.mono, fontSize: 12 }}>{shortModel(sess.model)}</td>
                      <td style={{ padding: '11px 16px 11px 0', color: C.textDim, fontFamily: C.mono, fontSize: 12 }}>{sess.turns}</td>
                      <td style={{ padding: '11px 16px 11px 0', color: C.textDim, fontFamily: C.mono, fontSize: 12 }}>{fmtTokens(sess.tokens)}</td>
                      <td style={{ padding: '11px 0', color: C.amber, fontFamily: C.mono, fontSize: 12, fontWeight: 600 }}>{fmt$(sess.cost)}</td>
                    </tr>
                  ))}
                  {sessions.length === 0 && (
                    <tr><td colSpan={6} style={{ padding: '32px 0', color: C.muted, textAlign: 'center', fontSize: 13 }}>
                      No sessions found
                    </td></tr>
                  )}
                </tbody>
      ...
      {/* Footer */}
      <div style={{ marginTop: 32, textAlign: 'center', color: C.muted, fontSize: 11, fontFamily: C.mono }}>
        ~/.claude/projects  ·  ~/.codex/sessions  ·  ~/.claude/stats-cache.json  ·  ~/.claude/history.jsonl  ·  local data only
      </div>
    </div>
  )
}
